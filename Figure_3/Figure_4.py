...
time_sopcr2=np.linspace(0,5, len(sopc_r1))
sopc=pd.concat([sopc_r1, sopc_r2], axis=1)

#print(sopc[1667:2048].mean())
...
box_parts = ax.boxplot((sopc[['TM1-TM6']][-417:].dropna().to_numpy().flatten()*10,),
...
box_parts = ax.boxplot((sopca[['TM1-TM6']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+2*sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:blue', hatch='////', clip_on=False)

box_parts = ax.boxplot((popca[['TM1-TM6']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:green',hatch='////', clip_on=False)

box_parts = ax.boxplot((dmpca[['TM1-TM6']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:red',hatch='////', clip_on=False)
    

custom_x = [5.55, 5.95, 6.35]  
custom_labels = ['DMPC', 'POPC', 'SOPC']
...
box_parts = ax1.boxplot((sopca[['TM3-TM6']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+2*sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:blue', hatch='////', clip_on=False)

box_parts = ax1.boxplot((popca[['TM3-TM6']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:green',hatch='////', clip_on=False)

box_parts = ax1.boxplot((dmpca[['TM3-TM6']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:red',hatch='////', clip_on=False)
...
box_parts = ax2.boxplot((sopca[['NpxxY']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+2*sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:blue', hatch='////', clip_on=False)

box_parts = ax2.boxplot((popca[['NpxxY']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:green',hatch='////', clip_on=False)

box_parts = ax2.boxplot((dmpca[['NpxxY']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:red',hatch='////', clip_on=False)
...
box_parts = ax3.boxplot((sopca[['N300_S115']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+2*sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:blue', hatch='////', clip_on=False)

box_parts = ax3.boxplot((popca[['N300_S115']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:green',hatch='////', clip_on=False)

box_parts = ax3.boxplot((dmpca[['N300_S115']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:red',hatch='////', clip_on=False)    
...
box_parts = ax6.boxplot((sopc[['L78_Y297']][1667:2084].dropna().to_numpy().flatten()*10,),
                        ...
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:blue',label='Inactive $@t=0$', clip_on=False)

box_parts = ax6.boxplot((popc[['L78_Y297']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sp,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:green',label='Inactive $@t=0$', clip_on=False)

box_parts = ax6.boxplot((dmpc[['L78_Y297']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:red',label='Inactive $@t=0$', clip_on=False)

for line in plt.gca().get_lines(): line.set_clip_on(False)
    
    
box_parts = ax6.boxplot((sopca[['L78_Y297']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+2*sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:blue', hatch='////',label='Inactive $@t=0$',  clip_on=False)

box_parts = ax6.boxplot((popca[['L78_Y297']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sp+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:green',hatch='////',label='Inactive $@t=0$',  clip_on=False)

box_parts = ax6.boxplot((dmpca[['L78_Y297']][1667:2084].dropna().to_numpy().flatten()*10,),
                        vert=True, positions=(start+sw,), widths=(width,),
                        medianprops=dict(color='k'), boxprops=dict(color='k'), showfliers=False, patch_artist=True, manage_ticks=False)
for patch in box_parts['boxes']:
    patch.set(facecolor='tab:red',hatch='////', label='Active $@t=0$', clip_on=False)    

custom_x = [5.55, 5.95, 6.35]  
custom_labels = ['DMPC', 'POPC', 'SOPC']
ax6.set_xticks(custom_x)
ax6.set_xticklabels(custom_labels)
#ax6.set_xticks([])
#ax6.axhline(y=0, lw=2,color='tab:red', label='DMPC')
#ax6.axhline(y=0, lw=2,color='tab:green', label='POPC')
#ax6.axhline(y=0, lw=2,color='tab:blue', label='SOPC')
...
leg=ax6.legend(ncol=1,frameon=False,borderpad=None, loc='upper center', bbox_to_anchor=(1.75,1.3), markerscale=8 ,columnspacing=1, handletextpad=0.5, handlelength=1.5, fontsize=12)

for i in leg.legendHandles:
    i.set_linewidth(2)

ax.text(-0.24, 1.07, 'a', transform=ax.transAxes, ha='center', fontsize=14, fontweight='bold')
ax1.text(-0.24, 1.07, 'b', transform=ax1.transAxes, ha='center', fontsize=14, fontweight='bold')
ax2.text(-0.24, 1.07, 'c', transform=ax2.transAxes, ha='center', fontsize=14, fontweight='bold')
ax3.text(-0.24, 1.07, 'd', transform=ax3.transAxes, ha='center', fontsize=14, fontweight='bold')
ax6.text(-0.24, 1.07, 'e', transform=ax6.transAxes, ha='center', fontsize=14, fontweight='bold')
fig = plt.gcf()


plt.subplots_adjust(top=0.90, bottom=0.115, left=0.08, right=0.94, hspace=0.43, wspace=0.45)
plt.Figure.set_size_inches(fig,(11, 5))
plt.savefig('Figure_3_re.png', dpi=600)
plt.savefig('Figure_3_re.svg', dpi=600)
#plt.close()
